...
输入参数：
	urg_t *urg				- 激光传感器变量
	long current_baudrate	- 当前波特率
	long next_baudrate		- 设置波特率
输出参数：
	int						- 出错信息
------------------------------------------------------*/
                                  ...
                                  long current_baudrate, long next_baudrate)
{
    ...
    // "SS" 僐儅儞僪偱儃乕儗乕僩傪曄峏偡傞
    _snprintf_s(buffer, SS_COMMAND_SIZE, "SS%06ld\n", next_baudrate);
                                 ...
                                 char scan_type_ch)
{
    ...
    if (urg->scanning_remain_times == 1) {
        ...
        write_size = _snprintf_s(buffer, BUFFER_SIZE, "%c%c%04d%04d%02d\n",
                              single_scan_ch, scan_type_ch,
                              urg->scanning_first_step + front_index,
                              urg->scanning_last_step + front_index,
                              urg->scanning_skip_step);
    } else {
        write_size = _snprintf_s(buffer, BUFFER_SIZE, "%c%c%04d%04d%02d%01d%02d\n",
...
int urg_laser_on(urg_t *urg)
{
    ...
    if (urg->is_laser_on != URG_FALSE) {
        // 婛偵儗乕僓偑敪岝偟偰偄傞偲偒偼丄僐儅儞僪傪憲怣偟側偄傛偆偵偡傞
        urg->last_errno = 0;
        return urg->last_errno;
    }
