...
static void swap_item(int from_index, int to_index)
{
    ...
    strncpy_s(buffer, found_ports[to_index], DEVICE_NAME_SIZE);
    strncpy_s(found_ports[to_index], found_ports[from_index], DEVICE_NAME_SIZE);
    strncpy_s(found_ports[from_index], buffer, DEVICE_NAME_SIZE);
...
int urg_serial_find_port(void)
{
    ...
    for (i = 0; SetupDiEnumDeviceInfo(hdi, i, &sDevInfo); ++i){
        ...
        _snprintf_s(found_ports[found_ports_size], DEVICE_NAME_SIZE, "%s", p);
